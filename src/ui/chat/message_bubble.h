...
// Adapted for zkgram: real tdesktop .h files get Qt/STL/base types for free
// from a shared PCH; this file is compiled directly into zkgram (no PCH),
// so the includes it implicitly relied on are made explicit here instead
// (base/basic_types.h itself is force-included project-wide already - see
// the /FI flag in the root CMakeLists.txt - but that alone does not cover
// Unexpected()/Expects(), Qt types, or std::array/vector/unique_ptr).
