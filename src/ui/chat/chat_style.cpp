...
#include "styles/style_zkgram_chat_style_stub.h" // was styles/style_polls.h - see zkgram_chat_style_stub.style
...
ChatStyle::ChatStyle(rpl::producer<ColorIndicesCompressed> colorIndices) {
	...
	// richPageStyle: real messageMarkdown/*Selected/*Out/*OutSelected
	// instances live in iv/iv.style (~830 lines: Instant View rich-page
	// rendering inside a message - tables, collapsible details, embedded
	// posts/related articles), a feature zkgram does not have - but this
	// field cannot be left default either (see zkgram_chat_style_stub.style's
	// own comment on defaultMarkdown: assignPalette() unconditionally reads
	// into it, a truly-default style::color crashes). zkgramMarkdownStub
	// gives it real, valid, palette-attached storage; the same stub is used
	// for all 4 variants since the content is never actually painted.
	make(
