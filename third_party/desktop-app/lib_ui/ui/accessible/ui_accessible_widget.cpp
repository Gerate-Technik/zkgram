...
QList<QAccessible::Attribute> Widget::attributeKeys() const {
	// zkgram patch: QAccessible::Attribute::Orientation does not exist in
	// vanilla Qt 6.9.3 (only Custom/Level are declared in qaccessible_base.h)
	// - it is only present in Telegram Desktop's own patched Qt build. This
	// is one narrow, cosmetic accessibility gap (screen readers do not get
	// an explicit orientation hint for this widget), not a functional or
	// security regression, and not worth vendoring a patched Qt for. See
	// zkgram/docs/README.md for the go/no-go decision this falls under.
